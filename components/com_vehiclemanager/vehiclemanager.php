...
// 			exit ();
        ...
        HTML_vehiclemanager :: showRentRequestThanks($params, $catid, $currentcat,$vehicleid);
    }
...
// print_r($query); exit;
        $vehicles = $database->loadObjectList();
         ...
         $query = "SELECT v.*,c.id, c.parent_id, c.title, c.published, c.image,COUNT(vc.iditem) as vehicles, '1' as display" .
              ...
              AND c.published=1  AND v.approved='1' $lang AND ({$s})
              \n GROUP BY c.id
              \n ORDER BY c.parent_id DESC, c.ordering ";
...
//   $menu_name = $menu1->getItem($Itemid)->title ;
        ...
        if ($currencys[count($currencys) - 1] == "")array_pop($currencys);
        $currencys1 = array();
        ...
        foreach ($currencys1 as $key => $row) {
            if ($vehicle->priceunit == $key)$currencys_one = $row;
        }
        
        if ($currencys_one != ""){
        foreach ($currencys1 as $key => $row) {
            $currencys_price[$key] = round($row / $currencys_one * $vehicle->price, 2);
            }
        }    
...
//print_r($max_pric.":111111111:".$stepFinalPrice."<br />");
    ...
    {
        global $mainframe, $database, $my, $acl, $hide_js, $langContent;
        global $mosConfig_shownoauth, $mosConfig_live_site, $mosConfig_absolute_path;
        global $cur_template, $Itemid, $vehiclemanager_configuration, $mosConfig_list_limit, $limit, $total, $limitstart;
...
//    $menu_name = $menu1->getItem($Itemid)->title ;
    ...
    static function showTabs()
    {
        ...
        if ($option != "com_vehiclemanager")
        {
            $comprofiler = "&tab=getmyvehiclesTab&is_show_data=1";
        } else
        {
            $comprofiler = "";
        }
    ...
    static function rent_requests($option, $vid)
    {
        ...
        PHP_vehiclemanager :: showTabs();
        HTML_vehiclemanager :: showRequestRentVehicles($option, $rent_requests, $pageNav, $Itemid);
    }
