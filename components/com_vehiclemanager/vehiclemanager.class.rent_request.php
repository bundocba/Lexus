   ...
   function accept()
    {
        ...
        $query = "SELECT fk_vehicleid FROM #__vehiclemanager_rent_request where fk_vehicleid = " . $this->fk_vehicleid;
        $data->setQuery($query);
        $v_assoc = $data->loadResult();
        //$v_assoc_id = explode(',', $v_assoc);
        $assoc_veh = getAssociateVehicle($v_assoc);
