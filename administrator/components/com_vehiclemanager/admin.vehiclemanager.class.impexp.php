        ...
        for ($i = 0; $i < $vehicle_list->length; $i++) {
          ...
          //get images
            ...
            if ($tmp[5] == "OK" && mosVehicleManagerImportExport::getXMLItemValue($vehicle, 'rent_sal') != "") {
                ...
                for ($j = 0; $j < $rent_list->length; $j++) {
                    ...
                    $database->setQuery("INSERT INTO #__vehiclemanager_rent_sal" .
                        ...
                        "', " . $help_comment_price . ",'" . $help_priceunit . "');");
                    $database->query();
                }
            }
            ...
            for ($j = 0; $j < $rent_list->length; $j++) {
                ...
                $order_email = mosVehicleManagerImportExport::getXMLItemValue($orders, 'email');
                $order_name = mosVehicleManagerImportExport::getXMLItemValue($orders, 'name');
                $order_status = mosVehicleManagerImportExport::getXMLItemValue($orders, 'status');
                $order_data = mosVehicleManagerImportExport::getXMLItemValue($orders, 'data');
                ...
                $database->setQuery("INSERT INTO #__vehiclemanager_orders " .
                        "\n (fk_user_id, email, name, status, data, fk_vehicle_id, payer_id, payer_status)" .
                        "\n VALUES " . " ('" . $order_userid .
                        "','" . $order_email .
                        "','" . $order_name .
                        "', '" . $order_status .
                        ...
                        "', '" . $order_payer_status . "');");
                $database->query();
                $database->setQuery("UPDATE #__vehiclemanager_orders SET fk_vehicle_id =" .
                 $vehicle_class->id . " WHERE fk_vehicle_id = " . $old_vehicle_id . "");
                $database->query();
            }

                     //get suggestion
            $suggestion_list = $vehicle->getElementsByTagname('suggestion');
            for ($j = 0; $j < $rent_list->length; $j++) {
                $suggestion = $suggestion_list->item($j);
                $suggestion_date = mosVehicleManagerImportExport::getXMLItemValue($suggestion, 'date');
                $suggestion_title = mosVehicleManagerImportExport::getXMLItemValue($suggestion, 'title');
                $suggestion_user_name = mosVehicleManagerImportExport::getXMLItemValue($suggestion, 'user_name');
                $suggestion_user_email = mosVehicleManagerImportExport::getXMLItemValue($suggestion, 'user_email');
                $suggestion_comment = mosVehicleManagerImportExport::getXMLItemValue($suggestion, 'comment');

                //insert data in table #__rem_suggestion
                $database->setQuery("INSERT INTO #__vehiclemanager_suggestion " .
                    "\n (date, title, name, user_name, user_email, comment)" .
                    "\n VALUES " . " ('" . $suggestion_date .
                    "','" . $suggestion_title .
...
static function exportVehicles($option) {
    ...
    if ($type == '4') {
        ...
        $database->setQuery("SELECT * FROM #__vehiclemanager_suggestion ");
    ...
    $strXmlDocSuggestion = "";
    $strXmlDocSuggestion.= "<suggestions>\n";
    if(!empty($suggestion)){
        foreach ($suggestion as $suggestion1) {
            $strXmlDocSuggestion .= "<suggestion>\n";
            foreach ($suggestion1 as $field => $value) {
                $strXmlDocSuggestion.= '<' . $field . '><![CDATA[' .
                 $value . ']]></' . $field . ">\n";
            }
        }
        $strXmlDocSuggestion .= "</suggestion>\n";
