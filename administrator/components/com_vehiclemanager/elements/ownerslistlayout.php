...
if (version_compare(JVERSION, "1.6.0", "lt"))
{
    ...
    class JElementOwnersListlayout extends JElement
    {

        var $_name = 'ownerslistlayout';

        function fetchElement($name, $value, &$node, $control_name)
        {
            $all_categories_layout = getLayoutsRem('com_vehiclemanager','owners_list');
            $layouts = Array();
...
} else if (version_compare(JVERSION, "1.6.0", "ge") && version_compare(JVERSION, "3.5.0", "lt"))
{
    ...
    class JFormFieldOwnersListlayout extends JFormField
    {

        protected function getInput()
        {
            $all_categories_layout = getLayoutsVeh('com_vehiclemanager','owners_list');
            $layouts = Array();
            $layouts[] = JHtml::_('select.option', '', 'Use Global');
            foreach($all_categories_layout as $value){
                $layouts[] = JHtml::_('select.option', "$value", "$value"); 
